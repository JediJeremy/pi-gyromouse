// a bunch of stuff I haven't yet got around to using... 


// I2Cdev library collection - MPU6050 I2C device class, 6-axis MotionApps 2.0 implementation
// Based on InvenSense MPU-6050 register map document rev. 2.0, 5/19/2011 (RM-MPU-6000A-00)
var MotionApps20 = {
		
	// this block of memory gets written to the MPU on start-up, and it seems
	// to be volatile memory, so it has to be done each time (it only takes ~1
	// second though)
	memory : [
	    // bank 0, 256 bytes
	    ,
	
	    // bank 1, 256 bytes
	    ,
	
	    // bank 2, 256 bytes
	    ,
	
	    // bank 3, 256 bytes
	    ,
	
	    // bank 4, 256 bytes
	    ,
	
	    // bank 5, 256 bytes
	    ,
	
	    // bank 6, 256 bytes
	    ,
	
	    // bank 7, 138 bytes (remainder)
	    
	],
	
	
	// thanks to Noah Zerkin for piecing this stuff together!
	config : [
		//  BANK    OFFSET  LENGTH  [DATA]
	    0x03,   0x7B,   0x03,   0x4C, 0xCD, 0x6C,         // FCFG_1 inv_set_gyro_calibration
	    0x03,   0xAB,   0x03,   0x36, 0x56, 0x76,         // FCFG_3 inv_set_gyro_calibration
	    0x00,   0x68,   0x04,   0x02, 0xCB, 0x47, 0xA2,   // D_0_104 inv_set_gyro_calibration
	    0x02,   0x18,   0x04,   0x00, 0x05, 0x8B, 0xC1,   // D_0_24 inv_set_gyro_calibration
	    0x01,   0x0C,   0x04,   0x00, 0x00, 0x00, 0x00,   // D_1_152 inv_set_accel_calibration
	    0x03,   0x7F,   0x06,   0x0C, 0xC9, 0x2C, 0x97, 0x97, 0x97, // FCFG_2 inv_set_accel_calibration
	    0x03,   0x89,   0x03,   0x26, 0x46, 0x66,         // FCFG_7 inv_set_accel_calibration
	    0x00,   0x6C,   0x02,   0x20, 0x00,               // D_0_108 inv_set_accel_calibration
	    0x02,   0x40,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_00 inv_set_compass_calibration
	    0x02,   0x44,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_01
	    0x02,   0x48,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_02
	    0x02,   0x4C,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_10
	    0x02,   0x50,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_11
	    0x02,   0x54,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_12
	    0x02,   0x58,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_20
	    0x02,   0x5C,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_21
	    0x02,   0xBC,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_22
	    0x01,   0xEC,   0x04,   0x00, 0x00, 0x40, 0x00,   // D_1_236 inv_apply_endian_accel
	    0x03,   0x7F,   0x06,   0x0C, 0xC9, 0x2C, 0x97, 0x97, 0x97, // FCFG_2 inv_set_mpu_sensors
	    0x04,   0x02,   0x03,   0x0D, 0x35, 0x5D,         // CFG_MOTION_BIAS inv_turn_on_bias_from_no_motion
	    0x04,   0x09,   0x04,   0x87, 0x2D, 0x35, 0x3D,   // FCFG_5 inv_set_bias_update
	    0x00,   0xA3,   0x01,   0x00,                     // D_0_163 inv_set_dead_zone
	                 // SPECIAL 0x01 = enable interrupts
	    0x00,   0x00,   0x00,   0x01, // SET INT_ENABLE at i=22, SPECIAL INSTRUCTION
	    0x07,   0x86,   0x01,   0xFE,                     // CFG_6 inv_set_fifo_interupt
	    0x07,   0x41,   0x05,   0xF1, 0x20, 0x28, 0x30, 0x38, // CFG_8 inv_send_quaternion
	    0x07,   0x7E,   0x01,   0x30,                     // CFG_16 inv_set_footer
	    0x07,   0x46,   0x01,   0x9A,                     // CFG_GYRO_SOURCE inv_send_gyro
	    0x07,   0x47,   0x04,   0xF1, 0x28, 0x30, 0x38,   // CFG_9 inv_send_gyro -> inv_construct3_fifo
	    0x07,   0x6C,   0x04,   0xF1, 0x28, 0x30, 0x38,   // CFG_12 inv_send_accel -> inv_construct3_fifo
	    0x02,   0x16,   0x02,   0x00, 0x01                // D_0_22 inv_set_fifo_rate
	
	    // This very last 0x01 WAS a 0x09, which drops the FIFO rate down to 20 Hz. 0x07 is 25 Hz,
	    // 0x01 is 100Hz. Going faster than 100Hz (0x00=200Hz) tends to result in very noisy data.
	    // DMP output frequency is calculated easily using this equation: (200Hz / (1 + value))
	
	    // It is important to make sure the host processor can keep up with reading and processing
	    // the FIFO output at the desired rate. Handling FIFO overflow cleanly is also a good idea.
	],
	
	updates : [
	    0x01,   0xB2,   0x02,   0xFF, 0xFF,
	    0x01,   0x90,   0x04,   0x09, 0x23, 0xA1, 0x35,
	    0x01,   0x6A,   0x02,   0x06, 0x00,
	    0x01,   0x60,   0x08,   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	    0x00,   0x60,   0x04,   0x40, 0x00, 0x00, 0x00,
	    0x01,   0x62,   0x02,   0x00, 0x00,
	    0x00,   0x60,   0x04,   0x00, 0x40, 0x00, 0x00
	],
	
	/*init: function() {
		// reset device
	    console.log("Resetting MPU6050...");
	    reset();
	    delay(30); // wait after reset

	    // disable sleep mode
	    console.log("Disabling sleep mode...");
	    setSleepEnabled(false);
	
	    // get MPU hardware revision
	    console.log("Selecting user bank 16...");
	    setMemoryBank(0x10, true, true);
	    console.log("Selecting memory byte 6...");
	    setMemoryStartAddress(0x06);
	    console.log("Checking hardware revision...");
	    uint8_t hwRevision = readMemoryByte();
	    DEBUG_PRINT();
	    console.log("Revision @ user[16][6] = ",hwRevision.toString(16));
	    console.log("Resetting memory bank selection to 0...");
	    setMemoryBank(0, false, false);
	
	    // check OTP bank valid
	    console.log("Reading OTP bank valid flag...");
	    uint8_t otpValid = getOTPBankValid();
	    console.log("OTP bank is ", otpValid ? "valid!" : "invalid!");
	
	    // get X/Y/Z gyro offsets
	    console.log("Reading gyro offset values...");
	    int8_t xgOffset = getXGyroOffset();
	    int8_t ygOffset = getYGyroOffset();
	    int8_t zgOffset = getZGyroOffset();
	    DEBUG_PRINT("X gyro offset = ");
	    console.log(xgOffset);
	    DEBUG_PRINT("Y gyro offset = ");
	    console.log(ygOffset);
	    DEBUG_PRINT("Z gyro offset = ");
	    console.log(zgOffset);
	
	    // setup weird slave stuff (?)
	    console.log("Setting slave 0 address to 0x7F...");
	    setSlaveAddress(0, 0x7F);
	    console.log("Disabling I2C Master mode...");
	    setI2CMasterModeEnabled(false);
	    console.log("Setting slave 0 address to 0x68 (self)...");
	    setSlaveAddress(0, 0x68);
	    console.log("Resetting I2C Master control...");
	    resetI2CMaster();
	    delay(20);
	
	    // load DMP code into memory banks
	    console.log("Writing DMP code to MPU memory banks.");
	    if (writeProgMemoryBlock(this.memory)) {
	        console.log("Success! DMP code written.");
	
	        // write DMP configuration
	        console.log("Writing DMP configuration to MPU memory banks.");
	        if (writeProgDMPConfigurationSet(this.config)) {
	            console.log("Success! DMP configuration written and verified.");
	
	            console.log("Setting clock source to Z Gyro...");
	            setClockSource(MPU6050_CLOCK_PLL_ZGYRO);
	
	            console.log("Setting DMP and FIFO_OFLOW interrupts enabled...");
	            setIntEnabled(0x12);
	
	            console.log("Setting sample rate to 200Hz...");
	            setRate(4); // 1khz / (1 + 4) = 200 Hz
	
	            console.log("Setting external frame sync to TEMP_OUT_L[0]...");
	            setExternalFrameSync(MPU6050_EXT_SYNC_TEMP_OUT_L);
	
	            console.log("Setting DLPF bandwidth to 42Hz...");
	            setDLPFMode(MPU6050_DLPF_BW_42);
	
	            console.log("Setting gyro sensitivity to +/- 2000 deg/sec...");
	            setFullScaleGyroRange(MPU6050_GYRO_FS_2000);
	
	            console.log("Setting DMP configuration bytes (function unknown)...");
	            setDMPConfig1(0x03);
	            setDMPConfig2(0x00);
	
	            console.log("Clearing OTP Bank flag...");
	            setOTPBankValid(false);
	
	            console.log("Setting X/Y/Z gyro offsets to previous values...");
	            setXGyroOffset(xgOffset);
	            setYGyroOffset(ygOffset);
	            setZGyroOffset(zgOffset);
	
	            console.log("Setting X/Y/Z gyro user offsets to zero...");
	            setXGyroOffsetUser(0);
	            setYGyroOffsetUser(0);
	            setZGyroOffsetUser(0);
	
	            console.log("Writing final memory update 1/7 (function unknown)...");
	            uint8_t dmpUpdate[16], j;
	            uint16_t pos = 0;
	            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
	            writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
	
	            console.log("Writing final memory update 2/7 (function unknown)...");
	            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
	            writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
	
	            console.log("Resetting FIFO..."))
	            resetFIFO();
	
	            console.log("Reading FIFO count...");
	            uint8_t fifoCount = getFIFOCount();
	            uint8_t fifoBuffer[128];
	
	            DEBUG_PRINT("Current FIFO count=");
	            console.log(fifoCount);
	            getFIFOBytes(fifoBuffer, fifoCount);
	
	            console.log("Setting motion detection threshold to 2...");
	            setMotionDetectionThreshold(2);
	
	            console.log("Setting zero-motion detection threshold to 156...");
	            setZeroMotionDetectionThreshold(156);
	
	            console.log("Setting motion detection duration to 80...");
	            setMotionDetectionDuration(80);
	
	            console.log("Setting zero-motion detection duration to 0...");
	            setZeroMotionDetectionDuration(0);
	
	            console.log("Resetting FIFO...");
	            resetFIFO();
	
	            console.log("Enabling FIFO...");
	            setFIFOEnabled(true);
	
	            console.log("Enabling DMP...");
	            setDMPEnabled(true);
	
	            console.log("Resetting DMP...");
	            resetDMP();
	
	            console.log("Writing final memory update 3/7 (function unknown)...");
	            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
	            writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
	
	            console.log("Writing final memory update 4/7 (function unknown)...");
	            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
	            writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
	
	            console.log("Writing final memory update 5/7 (function unknown)...");
	            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
	            writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
	
	            console.log("Waiting for FIFO count > 2...");
	            while ((fifoCount = getFIFOCount()) < 3);
	
	            DEBUG_PRINT("Current FIFO count=");
	            console.log(fifoCount);
	            console.log("Reading FIFO data...");
	            getFIFOBytes(fifoBuffer, fifoCount);
	
	            console.log("Reading interrupt status...");
	            uint8_t mpuIntStatus = getIntStatus();
	
	            console.log("Current interrupt status = ", mpuIntStatus.toString(16));
	
	            console.log("Reading final memory update 6/7 (function unknown)...");
	            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
	            readMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
	
	            console.log("Waiting for FIFO count > 2...");
	            while ((fifoCount = getFIFOCount()) < 3);
	
	            console.log("Current FIFO count=");
	            DEBUG_PRINTLN(fifoCount);
	
	            console.log("Reading FIFO data...");
	            getFIFOBytes(fifoBuffer, fifoCount);
	
	            console.log("Reading interrupt status...");
	            mpuIntStatus = getIntStatus();
	
	            console.log("Current interrupt status=", mpuIntStatus.toString(16));
	
	            console.log("Writing final memory update 7/7 (function unknown)...");
	            for (j = 0; j < 4 || j < dmpUpdate[2] + 3; j++, pos++) dmpUpdate[j] = pgm_read_byte(&dmpUpdates[pos]);
	            writeMemoryBlock(dmpUpdate + 3, dmpUpdate[2], dmpUpdate[0], dmpUpdate[1]);
	
	            console.log("DMP is good to go! Finally.");
	
	            console.log("Disabling DMP (you turn it on later)...");
	            setDMPEnabled(false);
	
	            console.log("Setting up internal 42-byte (default) DMP packet buffer...");
	            dmpPacketSize = 42;
	
	            console.log("Resetting FIFO and clearing INT status one last time...");
	            resetFIFO();
	            getIntStatus();
	        } else {
	            console.log("ERROR! DMP configuration verification failed.");
	            return 2; // configuration block loading failed
	        }
	    } else {
	        console.log("ERROR! DMP code verification failed.");
	        return 1; // main binary block loading failed
	    }
	    return; // success
	}*/

};

// I2Cdev library collection - MPU6050 I2C device class, 9-axis MotionApps 4.1 implementation
// Based on InvenSense MPU-6050 register map document rev. 2.0, 5/19/2011 (RM-MPU-6000A-00)
var MotionApps41 = {
	memory: [
	    // bank 0, 256 bytes
	    ,
	
	    // bank 1, 256 bytes
	    ,
	    
	    // bank 2, 256 bytes
	    ,
	    
	    // bank 3, 256 bytes
	    ,
	    
	    // bank 4, 256 bytes
	    ,
	    
	    // bank 5, 256 bytes
	    ,
	
	    // bank 6, 256 bytes
	    ,
	    
	    // bank 7, 170 bytes (remainder)
	    
    ],
	config: [
		//  BANK    OFFSET  LENGTH  [DATA]
	    0x02,   0xEC,   0x04,   0x00, 0x47, 0x7D, 0x1A,   // ?
	    0x03,   0x82,   0x03,   0x4C, 0xCD, 0x6C,         // FCFG_1 inv_set_gyro_calibration
	    0x03,   0xB2,   0x03,   0x36, 0x56, 0x76,         // FCFG_3 inv_set_gyro_calibration
	    0x00,   0x68,   0x04,   0x02, 0xCA, 0xE3, 0x09,   // D_0_104 inv_set_gyro_calibration
	    0x01,   0x0C,   0x04,   0x00, 0x00, 0x00, 0x00,   // D_1_152 inv_set_accel_calibration
	    0x03,   0x86,   0x03,   0x0C, 0xC9, 0x2C,         // FCFG_2 inv_set_accel_calibration
	    0x03,   0x90,   0x03,   0x26, 0x46, 0x66,         //   (continued)...FCFG_2 inv_set_accel_calibration
	    0x00,   0x6C,   0x02,   0x40, 0x00,               // D_0_108 inv_set_accel_calibration
	
	    0x02,   0x40,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_00 inv_set_compass_calibration
	    0x02,   0x44,   0x04,   0x40, 0x00, 0x00, 0x00,   // CPASS_MTX_01
	    0x02,   0x48,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_02
	    0x02,   0x4C,   0x04,   0x40, 0x00, 0x00, 0x00,   // CPASS_MTX_10
	    0x02,   0x50,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_11
	    0x02,   0x54,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_12
	    0x02,   0x58,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_20
	    0x02,   0x5C,   0x04,   0x00, 0x00, 0x00, 0x00,   // CPASS_MTX_21
	    0x02,   0xBC,   0x04,   0xC0, 0x00, 0x00, 0x00,   // CPASS_MTX_22
	
	    0x01,   0xEC,   0x04,   0x00, 0x00, 0x40, 0x00,   // D_1_236 inv_apply_endian_accel
	    0x03,   0x86,   0x06,   0x0C, 0xC9, 0x2C, 0x97, 0x97, 0x97, // FCFG_2 inv_set_mpu_sensors
	    0x04,   0x22,   0x03,   0x0D, 0x35, 0x5D,         // CFG_MOTION_BIAS inv_turn_on_bias_from_no_motion
	    0x00,   0xA3,   0x01,   0x00,                     // ?
	    0x04,   0x29,   0x04,   0x87, 0x2D, 0x35, 0x3D,   // FCFG_5 inv_set_bias_update
	    0x07,   0x62,   0x05,   0xF1, 0x20, 0x28, 0x30, 0x38, // CFG_8 inv_send_quaternion
	    0x07,   0x9F,   0x01,   0x30,                     // CFG_16 inv_set_footer
	    0x07,   0x67,   0x01,   0x9A,                     // CFG_GYRO_SOURCE inv_send_gyro
	    0x07,   0x68,   0x04,   0xF1, 0x28, 0x30, 0x38,   // CFG_9 inv_send_gyro -> inv_construct3_fifo
	    0x07,   0x62,   0x05,   0xF1, 0x20, 0x28, 0x30, 0x38, // ?
	    0x02,   0x0C,   0x04,   0x00, 0x00, 0x00, 0x00,   // ?
	    0x07,   0x83,   0x06,   0xC2, 0xCA, 0xC4, 0xA3, 0xA3, 0xA3, // ?
	                 // SPECIAL 0x01 = enable interrupts
	    0x00,   0x00,   0x00,   0x01, // SET INT_ENABLE, SPECIAL INSTRUCTION
	    0x07,   0xA7,   0x01,   0xFE,                     // ?
	    0x07,   0x62,   0x05,   0xF1, 0x20, 0x28, 0x30, 0x38, // ?
	    0x07,   0x67,   0x01,   0x9A,                     // ?
	    0x07,   0x68,   0x04,   0xF1, 0x28, 0x30, 0x38,   // CFG_12 inv_send_accel -> inv_construct3_fifo
	    0x07,   0x8D,   0x04,   0xF1, 0x28, 0x30, 0x38,   // ??? CFG_12 inv_send_mag -> inv_construct3_fifo
	    0x02,   0x16,   0x02,   0x00, 0x03                // D_0_22 inv_set_fifo_rate
	
	    // This very last 0x01 WAS a 0x09, which drops the FIFO rate down to 20 Hz. 0x07 is 25 Hz,
	    // 0x01 is 100Hz. Going faster than 100Hz (0x00=200Hz) tends to result in very noisy data.
	    // DMP output frequency is calculated easily using this equation: (200Hz / (1 + value))
	
	    // It is important to make sure the host processor can keep up with reading and processing
	    // the FIFO output at the desired rate. Handling FIFO overflow cleanly is also a good idea.
	],
	updates:[
	    0x01,   0xB2,   0x02,   0xFF, 0xF5,
	    0x01,   0x90,   0x04,   0x0A, 0x0D, 0x97, 0xC0,
	    0x00,   0xA3,   0x01,   0x00,
	    0x04,   0x29,   0x04,   0x87, 0x2D, 0x35, 0x3D,
	    0x01,   0x6A,   0x02,   0x06, 0x00,
	    0x01,   0x60,   0x08,   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	    0x00,   0x60,   0x04,   0x40, 0x00, 0x00, 0x00,
	    0x02,   0x60,   0x0C,   0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	    0x01,   0x08,   0x02,   0x01, 0x20,
	    0x01,   0x0A,   0x02,   0x00, 0x4E,
	    0x01,   0x02,   0x02,   0xFE, 0xB3,
	    0x02,   0x6C,   0x04,   0x00, 0x00, 0x00, 0x00, // READ
	    0x02,   0x6C,   0x04,   0xFA, 0xFE, 0x00, 0x00,
	    0x02,   0x60,   0x0C,   0xFF, 0xFF, 0xCB, 0x4D, 0x00, 0x01, 0x08, 0xC1, 0xFF, 0xFF, 0xBC, 0x2C,
	    0x02,   0xF4,   0x04,   0x00, 0x00, 0x00, 0x00,
	    0x02,   0xF8,   0x04,   0x00, 0x00, 0x00, 0x00,
	    0x02,   0xFC,   0x04,   0x00, 0x00, 0x00, 0x00,
	    0x00,   0x60,   0x04,   0x40, 0x00, 0x00, 0x00,
	    0x00,   0x60,   0x04,   0x00, 0x40, 0x00, 0x00
    ],
    init: function() {
		// not done, can't test
	}
};
